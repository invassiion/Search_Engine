...
public class GetSearchDto implements Callable {
    ...
    private final Map.Entry<PageEntity, Float> entry;
    private final Set<String> entityLemmas;
    private final Morphology morphology;

    private final LemmaService lemmaService;

    public GetSearchDto(Map.Entry<PageEntity, Float> entry, Set<String> entityLemmas, Morphology morphology, LemmaService lemmaService) {
        this.entry = entry;
    ...
    public SearchDto call() throws Exception {
        ...
        searchDto.setSite(entry.getKey().getSiteId().getUrl());
        searchDto.setSiteName(entry.getKey().getSiteId().getName());
        StringBuilder stringBuilder = new StringBuilder();

        String body = clear(entry.getKey().getContent(), "body");
    ...
    private String getSnippet(String content, Set<String> lemmaList) throws IOException {
        List<Integer> lemmaIndex = new ArrayList<>();
        StringBuilder result = new StringBuilder();
        for (String lemma : lemmaList) {
            lemmaIndex.addAll(lemmaService.findLemmaIndexInText(content, lemma));
        }
    ...
    private String getTitle(String body) {
        String title = clear(entry.getKey().getContent(), "title");
        if (title.length() < 2) {
